...
function App() {
  ...
  useEffect(() => {
    fetch("https://dull-blue-basket-clam-wrap.cyclic.app/pets_price")
      .then((res) => res.json())
      .then((result) => {
        console.log(result);
        setPetPrice(result);
    ...
    fetch("https://dull-blue-basket-clam-wrap.cyclic.app/pets_price_chart")
      .then((res) => res.json())
      .then((result) => {
        console.log(result);
        setPetChart({
  ...
  return (
    <div>
      <h1>Mekhin Mung-unklang 6400898</h1>
      <ul>
        {petPrices.map((pet) => (
          <li key={pet.id}>
